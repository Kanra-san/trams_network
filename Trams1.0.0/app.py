...
def optimize_schedule():
    try:
        ...
        # Map frontend day_type to Polish XML string
        day_type_map = {
            'workday': 'w dni robocze',
            'saturday': 'Sobota',
            'sunday': 'Niedziela'
        }
        original_day_type = day_type
        day_type = day_type_map.get(day_type.lower(), day_type)
        print(f"[API] Mapped day_type '{original_day_type}' to '{day_type}' for optimizer.")
        ...
        # --- Add stop name mapping for results ---
        stop_id_to_name = {sid: sname for sid, sname in db.get_stops_with_names_and_ids()}
        def stop_label(stop_id):
            name = stop_id_to_name.get(str(stop_id), str(stop_id))
            return f"{name} ({stop_id})" if name != str(stop_id) else str(stop_id)

        # Convert optimization result to JSON-serializable format and add readable fields
        if isinstance(optimization_result, pd.DataFrame):
            print(f"[API] DataFrame shape: {optimization_result.shape}")
            print(f"[API] DataFrame head:\n{optimization_result.head()}\n")
            df = optimization_result.copy()
            df = df.rename(columns={
                "stop_id": "Stop",
                "hour": "Hour",
                "original_trams": "Current Trips",
                "congestion_percent": "Congestion (%)",
                "suggested_trams": "Optimized Trips"
            })
            # Replace Stop column with name(id) style
            if "Stop" in df.columns:
                df["Stop"] = df["Stop"].apply(stop_label)
            # Only include columns that exist in the DataFrame (remove Day)
            desired_columns = ["Stop", "Hour", "Current Trips", "Congestion (%)", "Optimized Trips"]
            columns = [col for col in desired_columns if col in df.columns]
            df = df[columns]
            print(f"[API] Final dict sample: {df.head(3).to_dict(orient='records')}")
            optimization_result = df.to_dict(orient='records')
