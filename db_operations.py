...
class TramDatabaseOperations:
    ...
    def create_network_graph(self):
        """Create a network graph from the database"""
        with self._get_connection() as conn:
            cursor = conn.cursor()

            # Get all stops with coordinates and active status
            cursor.execute('''
            ...
            for stop_id, stop_name, lat, lon, active_status in cursor.fetchall():
                G.add_node(stop_id,
                           ...
                           pos=(float(lat), float(lon)) if lat and lon else None)

            # Get all connections between active stops
            cursor.execute('''
